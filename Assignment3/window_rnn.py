...
class WindowRNN(RNN):
    ...
    def forward(self, inputs: list[npt.NDArray[np.float64]]) -> float:
        ...
        # Step through each window and calculate hidden states
        for window_start_index in range(len(inputs) - self.window_size + 1):
            window = inputs[window_start_index : window_start_index + self.window_size]
